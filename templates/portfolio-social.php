...
foreach ($portfolio_array as $port) {
	if ($port['icon']) {
		...
		$portDetails .= '<div class="portfolio-detail" id="'. cleanLinkTitle($port['title']) .'"><a href="#" class="boxclose" data-client="'. cleanLinkTitle($port['title']) .'"></a>';
		
		if ($port['icon']['sizes']['portfolio-logo-large']) {
			$portDetails .= (($port['icon']) ? '<img src="'. $upload_path . '/'. $port['icon']['sizes']['portfolio-logo-large']['file'] .'" class="logo img-responsive" alt="'. $port['icon-alt'] .'">' : '');
		} else {
...
function add_port_script() {
	echo '<script src="'. get_stylesheet_directory_uri() .'/js/portfolio.min.js"></script>';
}
